...
export function detectFormat(content: string): 'ancestry' | 'myheritage' | 'livingdna' | 'unknown' {
...
export function parseLivingDNAFile(content: string, sourceFile: 1 | 2 = 1): ParseResult {
  const lines = content.split('\n')
  const snps: SNP[] = []
  const errors: SkippedEntry[] = []
  let headerFound = false
