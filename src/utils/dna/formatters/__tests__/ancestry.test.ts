...
describe('generateAncestryCsv', () => {
  ...
  it('should handle single-character genotypes from 23andMe', async () => {
    ...
    expect(tsv).toContain('rs1\t23\t100\tA\tA') // A → AA split into A A
    expect(tsv).toContain('rs2\t24\t200\tT\tT') // T → TT split into T T
    expect(tsv).toContain('rs3\t26\t300\tC\tC') // C → CC split into C C
    expect(tsv).toContain('rs4\t23\t400\tG\tG') // G → GG split into G G
  })
})
